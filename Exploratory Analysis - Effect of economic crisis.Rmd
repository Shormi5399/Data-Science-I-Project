...
What information/features/characteristics do you have for each observation?
```{r}
labor_data_numeric <- labor_data %>% select(gdp,unemp,CPI) %>% filter(Year>=2007 & Year<=2015 & Country.Code == "IND")
cor(labor_data_numeric, use="complete")
```

```{r}
labor_data_numeric <- labor_data %>% select(gdp,unemp,CPI) %>% filter(Year>=2007 & Year<=2015 & Country.Code == "USA")
cor(labor_data_numeric, use="complete")
```

```{r}
labor_data_numeric <- labor_data %>% select(gdp,unemp,CPI) %>% filter(Year>=2007 & Year<=2015 & Country.Code == "CHN")
cor(labor_data_numeric, use="complete")
```
...
```{r}
labor_data_outlier <- labor_data %>% filter(CPI>600)
labor_data_outlier
```
...
ggplot(cpi_data_1,aes(x = Year, y = CPI, col = Country.Code, group = Country.Code)) +geom_line(na.rm = T,lwd = 1)
...
ggplot(unemployment_data_1, aes(x = Year, y = unemp,col = Country.Code, group = Country.Code))+geom_line(na.rm = T,lwd = 1)
```
