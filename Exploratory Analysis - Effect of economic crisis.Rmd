...
```{r}
trends.usa <-labor_data %>% select(Year,unemp,Country.Code) %>% filter(Country.Code == "USA")  
trendsts.usa <- ts(as.vector(trends.usa$unemp),start = c(1991,1), end = c(2014), frequency = 1)

trends.usabc <- labor_data  %>% select(Year,unemp,Country.Code) %>% filter(Country.Code == "USA" & Year >=1991 & Year<=2007)
