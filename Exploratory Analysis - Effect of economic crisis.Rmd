...
```{r}
trends.usa <- na.trim(labor_data) %>% select(Year,unemp,Country.Code) %>% filter(Country.Code == "USA")  
trendsts.usa <- ts(as.vector(trends.usa$unemp),start = c(1991,1), end = c(2014), frequency = 1)

trends.usabc <- na.trim(labor_data) %>% select(Year,unemp,Country.Code) %>% filter(Country.Code == "USA" & Year >=1991 & Year<=2007)
trendsts.usabc <- ts(as.vector(trends.usabc$unemp),start = c(1991,1), end = c(2007), frequency = 1)
...
```{r}
trends.ind <- labor_data %>% select(Year,unemp,Country.Code) %>% filter(Country.Code == "IND")
trendsts.ind <- ts(as.vector(trends.ind$unemp),start = c(1991,1), end = c(2014), frequency = 1)

trends.indbc <- labor_data %>% select(Year,unemp,Country.Code) %>% filter(Country.Code == "IND" & Year >=1991 & Year<=2007)
trendsts.indbc <- ts(as.vector(trends.indbc$unemp),start = c(1991,1), end = c(2007), frequency = 1)

auto.arima(trendsts.ind)
trends.arimaind<- arima(trendsts.ind, order=c(1,2,1))
trends.arimaind
trend.preds<-forecast.Arima(trends.arimaind,h=12)
plot.forecast(trend.preds,xlim = c(2000,2009),ylim = c(3,5))
summary(trend.preds)

auto.arima(trendsts.indbc)
trends.arimaind<- arima(trendsts.indbc, order=c(1,2,1))
trends.arimaind
trend.preds<-forecast.Arima(trends.arimaind,h=12)
plot.forecast(trend.preds,xlim = c(2000,2009),ylim = c(3,5))
summary(trend.preds)
...
```{r}
trends.usa <- labor_data %>% select(Year,CPI,Country.Code) %>% filter(Country.Code == "USA")
trendsts.usa <- ts(as.vector(trends.usa$CPI),start = c(1991,1), end = c(2014), frequency = 1)

trends.usabc <- labor_data %>% select(Year,CPI,Country.Code) %>% filter(Country.Code == "USA" & Year >=1991 & Year<=2007)
trendsts.usabc <- ts(as.vector(trends.usabc$CPI),start = c(1991,1), end = c(2007), frequency = 1)

auto.arima(trendsts.usa)
trends.arimausa<- arima(trendsts.usa, order=c(1,2,1))
trends.arimausa
trend.preds<-forecast.Arima(trends.arimausa,h=12)
plot.forecast(trend.preds,xlim = c(1991,2020),ylim = c(50,150))
summary(trend.preds)
